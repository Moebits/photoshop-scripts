const magicWandTool = (x: number, y: number) => {
...
const removeBackground = () => {
    ...
    if ((document.activeLayer as ArtLayer).isBackgroundLayer) document.activeLayer.name = "Layer 0"
    const originalRulerUnits = app.preferences.rulerUnits
    app.preferences.rulerUnits = Units.PIXELS
    document.selection.deselect()
    const height = document.height as unknown as number
    const width = document.width as unknown as number
    magicWandTool(10, 10)
...
const selectionFrame = (index: number) => {
